...
// Without this, a tenant could put something like a hidden script tag into their
// ticket title, and it would run as real code in the browser of whoever opens this
// confirmation page — which is the property manager, clicking from their email.
                ...
                <body>
                  <div class="card">
                      <div class="stamp">On Record</div>
                          <h1>${safeTitle}</h1>
                              <p>${safeMessage}</p>
                                  ${safeTicketId ? `<div class="id">${safeTicketId}${safeStatus ? ' — ' + safeStatus : ''}</div>` : ''}
                                    </div>
                                    </body>
                                    </html>`;
}
